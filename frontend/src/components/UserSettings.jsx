...
class UserSettings extends React.Component {
    ...
    componentDidMount() {
        Axios.get("/api/getcurrentuser", { withCredentials: true })
            .then(res => {
                // User is logged in; you can optionally set state with user info
            })
            .catch(() => {
    ...
    deleteAccount = (e) => {
        e.preventDefault();
        let confirmDelete = window.confirm("Are you sure you want to delete your account? THIS CANNOT BE UNDONE. ALL OF YOUR POSTS WILL BE DELETED");
        if (confirmDelete) {
                    ...
                    <div className="w3-container">
                        ...
                        {this.state.currentSetting === "main" && (
                            <div style={{ margin: "1rem" }}>
                                ...
                                <ul className="w3-ul w3-border w3-hoverable">
                                    <li onClick={() => this.setState({ currentSetting: "cpwd" })} style={{ cursor: "pointer" }} className="w3-hover-light-gray">
                                        Change password
                                    </li>
                                    <li onClick={() => this.setState({ currentSetting: "del" })} style={{ cursor: "pointer" }} className="w3-text-red w3-hover-pale-red w3-hover-text-red">
                                        Delete account
                                    </li>
                                </ul>
                            </div>
                        ...
                        {this.state.currentSetting === "cpwd" && (
                            <div style={{ margin: "1rem" }}>
                                ...
                                <hr className="w3-border-top w3-border-black" />
                                <button className="w3-button w3-blue" onClick={() => this.setState({ currentSetting: "main" })}>
                                    &laquo; Back
                                </button>
